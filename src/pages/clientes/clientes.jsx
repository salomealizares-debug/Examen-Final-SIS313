...
function ExtraRow({ name, index }) {
  return (
    <li className="ledger-row">
      <span className="ledger-row__index">{String(index).padStart(3, '0')}</span>
...
function Rubro({ code, title, accent, image, count, extra, query }) {
  ...
  const filteredExtra = extra
    ? extra.filter((name) => name.toLowerCase().includes(query.toLowerCase()))
    : []
          ...
          {filteredExtra.length > 0 ? (
            <ul className="ledger-list">
              {filteredExtra.map((name) => (
                <ExtraRow key={name} name={name} index={extra.indexOf(name) + count + 1} />
              ))}
            </ul>
          ) : (
            <p className="rubro__extra-empty">Sin coincidencias para "{query}".</p>
          )}
        </div>
      )}
    </section>
  ...
  return (
    <div className="clientes-directory">
      ...
      <header className="clientes-directory__header">
        <span className="clientes-directory__eyebrow">Directorio · Actualizado 2026</span>

        <h2 className="clientes-directory__title">Empresas Clientes</h2>
